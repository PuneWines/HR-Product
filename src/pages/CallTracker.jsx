...
const CallTracker = () => {
    ...
    try {
      ...
      const processedEnquiryData = enquiryDataFromRow7
        ...
        .filter(item => {
          ...
          const status = item.row[24] || ''; // Column Y (Status)
          return planned && status !== 'Joining';
        })
        .map(item => ({
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 6) {
        ...
        employeeIds.forEach(id => {
          ...
          if (!isNaN(num) && num > maxEmpId) maxEmpId = num;
        });

        // Indent No (REC-xx)
        const indentNos = dataRows.map(row => row[2]).filter(id => id && id.startsWith('REC-'));
        let maxIndentId = 0;
        indentNos.forEach(id => {
          const num = parseInt(id.replace('REC-', ''));
          if (!isNaN(num) && num > maxIndentId) maxIndentId = num;
        });

        // Enquiry No (ENQ-xx)
        const enquiryNos = dataRows.map(row => row[3]).filter(id => id && id.startsWith('ENQ-'));
        let maxEnquiryId = 0;
        enquiryNos.forEach(id => {
          const num = parseInt(id.replace('ENQ-', ''));
          if (!isNaN(num) && num > maxEnquiryId) maxEnquiryId = num;
        });

        return {
          employeeId: `JN-${String(maxEmpId + 1).padStart(3, '0')}`,
          indentNo: `REC-${String(maxIndentId + 1).padStart(2, '0')}`,
          enquiryNo: `ENQ-${String(maxEnquiryId + 1).padStart(2, '0')}`
        };
      }
      return {
        employeeId: 'JN-001',
      ...
      console.error('Error generating next IDs:', error);
      return {
        employeeId: 'JN-001',
        indentNo: 'REC-01',
        enquiryNo: 'ENQ-01'
  ...
  const uploadFileToDrive = async (originalFile, folderId) => {
    try {
      ...
      params.append('sheetName', 'JOINING'); // Fixes undefined.replace error in Apps Script
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.status === 'Joining') {
        ...
        setShowModal(false);
        const nextIds = await generateNextIds();
        setNextEmployeeId(nextIds.employeeId);
        setNextIndentNo(nextIds.indentNo);
        setNextEnquiryNo(nextIds.enquiryNo);
  ...
  const handleJoiningSubmit = async (e) => {
    ...
    try {
      ...
      rowData[2] = nextIndentNo;                  // Indent No
      rowData[3] = nextEnquiryNo;                 // Candidate Enquiry No
