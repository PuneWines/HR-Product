...
const MySalary = () => {
  ...
  const fetchSalaryData = async () => { 
    ...
    try {
      ...
      // PAID Record usually starts from index 1 (Row 2 is first data row)
      const dataRows = rawData.length > 1 ? rawData.slice(1) : [];

      // Finding column indices dynamically from headers (better than hardcoded)
      ...
      const dojIdx = getIdx('DOJ');
      const monthSalIdx = getIdx('Monthly Salary');
      const daysIdx = getIdx('Days in a Month');
      const adjIdx = getIdx('Mgmt Adjustment');
      const presentIdx = getIdx('Total Present');
      const advIdx = getIdx('Advance Deduction');
      const brackIdx = getIdx('Brackage');
      const medIdx = getIdx('Medical');
      ...
      const processedData = dataRows
        .map((row, index) => {
          return {
            id: index + 1,
            employeeId: (empIdIdx !== -1 ? row[empIdIdx] : '') || '',
            employeeName: (nameIdx !== -1 ? row[nameIdx] : '') || '',
            designation: (desigIdx !== -1 ? row[desigIdx] : '') || '',
            location: (locIdx !== -1 ? row[locIdx] : '') || '',
            doj: (dojIdx !== -1 ? row[dojIdx] : '') || '',
            monthlySalary: toNumber(monthSalIdx !== -1 ? row[monthSalIdx] : 0),
            daysInMonth: (daysIdx !== -1 ? row[daysIdx] : '') || '',
            mgmtAdjustment: (adjIdx !== -1 ? row[adjIdx] : '') || '',
            totalPresent: (presentIdx !== -1 ? row[presentIdx] : '') || '',
            advanceDeduction: toNumber(advIdx !== -1 ? row[advIdx] : 0),
            brackage: (brackIdx !== -1 ? row[brackageIdx] : '') || (brackIdx !== -1 ? row[brackIdx] : ''),
            medical: (medIdx !== -1 ? row[medIdx] : '') || '',
            totalSalary: toNumber(totalSalIdx !== -1 ? row[totalSalIdx] : 0),
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Advance Deduction" value={filteredSalary[0]?.advanceDeduction || 0} icon={ArrowDownRight} colorClass="bg-red-500" />
